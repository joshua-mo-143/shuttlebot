...
#[poise::command(slash_command)]
pub async fn resolve(ctx: Context<'_>,
    #[description = "ID of user who resolved the issue"] resolved_by: Option<u64>) -> Result<(), Error> {
    ...
    let name = if resolved_by.is_some() 
        {ctx.http().get_user(resolved_by.unwrap()).await.unwrap().name}
            else
